...
function finish {
  ...
  kubectl delete -f tests/test_crs/valid_fs_drift.yaml || echo CR delete did not happen
  delete_operator
}
...
function functional_test_fs_drift {
  ...
  sleep 30
  # if the run did not complete successfully, tell them so
  (kubectl logs "$fsdrift_pod" --namespace my-ripsaw | grep "RUN STATUS DONE") \
  || (echo 'fs-drift test: FAILED' ; exit 1)
